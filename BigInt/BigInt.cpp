...
bool BigInt::operator==(const long& other) const {
    return this->operator==(BigInt(other));
}
...
BigInt BigInt::power(const BigInt& dgr) const {
    if (dgr < 0) {
...
BigInt BigInt::recPower(const BigInt& dgr) const {
    if (dgr <= 10) {
        BigInt res = 1;

        for (BigInt i = 0; i < dgr; i = i + 1) {
            res = (*this) * res;
        }
    ...
    auto divRes = dgr.absDivide(10);
    BigInt tmp1 = this->recPower(divRes.first), tmp2 = this->recPower(divRes.second);
    BigInt s2 = tmp1 * tmp1;
    BigInt s4 = s2 * s2;

    return s4 * s4 * s2 * tmp2;
}
